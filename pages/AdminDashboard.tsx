...
import type { Order, Voucher, User, Settings, SupportTicket, OrderEditRequest } from '../types';
...
import SupportTicketList from './admin/SupportTicketList';
import EditRequestList from './admin/EditRequestList';
...
interface AdminDashboardProps {
    ...
    onRejectEditRequest: (requestId: string) => void;
}
...
const AdminDashboard: React.FC<AdminDashboardProps> = (props) => {
    ...
    const pendingSupportTickets = useMemo(() => props.supportTickets.filter(t => t.status === 'Đang xử lý').length, [props.supportTickets]);
    ...
    const renderContent = () => {
        switch (activeView) {
            ...
            case 'edit-requests':
                return <EditRequestList requests={props.orderEditRequests} onApprove={props.onApproveEditRequest} onReject={props.onRejectEditRequest} />;
            case 'users':
            ...
            case 'support':
                return <SupportTicketList tickets={props.supportTickets} onUpdateStatus={props.onUpdateTicketStatus}/>;
            case 'vouchers':
    ...
    const SidebarContent = () => (
         <div className="flex flex-col justify-between h-full">
            <div>
                 <div className="flex items-center gap-2 p-4 border-b border-slate-700 h-16">
                ...
                <nav className="flex flex-col p-4 space-y-2">
                    {menuItems.map(item => (
                        <button
                            ...
                            className={`relative flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium transition-all duration-200 ${ activeView === item.id ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-300 hover:bg-slate-700 hover:text-white' }`}
                        >
                            {item.icon}
                            <span>{item.label}</span>
                            {item.badge > 0 && (
                                <span className="absolute right-2 top-1/2 -translate-y-1/2 w-5 h-5 bg-red-500 rounded-full text-white text-xs flex items-center justify-center ring-2 ring-slate-800">{item.badge}</span>
                            )}
                        </button>
                    ))}
                    <hr className="border-slate-700 my-2" />
                     <button
                        ...
                        className="flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium text-slate-300 hover:bg-slate-700 hover:text-white transition-all duration-200"
                    >
            ...
            <div className="p-4 border-t border-slate-700">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-400">{props.user.email}</p>
                    </div>
                    <button onClick={props.onLogout} className="ml-auto p-2 text-slate-400 hover:text-white hover:bg-slate-700 rounded-md transition-colors">
    ...
    return (
        <>
            <div className="min-h-screen w-full flex bg-slate-100 dark:bg-gray-950">
                {isSidebarOpen && <div className="fixed inset-0 bg-black/60 z-30 lg:hidden" onClick={() => setIsSidebarOpen(false)}></div>}
                
                <aside className={`fixed lg:relative lg:translate-x-0 top-0 left-0 h-full z-40 w-64 bg-slate-800 dark:bg-slate-900 border-r border-slate-700 dark:border-slate-800 transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                    <SidebarContent />
                </aside>

                <div className="flex-1 flex flex-col w-full min-w-0">
                    <header className="flex items-center justify-between h-16 px-4 sm:px-8 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 backdrop-blur-sm lg:bg-transparent lg:dark:bg-transparent">
                        <button className="lg:hidden text-slate-600 dark:text-slate-300" onClick={() => setIsSidebarOpen(!isSidebarOpen)}>
                            <MenuIcon className="w-6 h-6"/>
                        </button>
                        <h1 className="text-xl font-semibold text-slate-800 dark:text-slate-200 lg:hidden">
            ...
            {selectedOrder && <OrderDetailsModal order={selectedOrder} onClose={() => setSelectedOrder(null)} onSave={props.onUpdateOrderDetails} vouchers={props.vouchers} />}
            {userToBan && <BanUserModal user={userToBan} onClose={() => setUserToBan(null)} onConfirmBan={handleConfirmBan} addToast={props.addToast} />}
